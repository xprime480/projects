  ...
  public std::iterator<std::input_iterator_tag, typename Container::value_type>
{
  ...
  {
    sequence_adapter_iterator t = *this;
    end = ! container->has_next();
    if ( ! end ) {
      value = container->next();
      ++count;
		       ...
		       typename std::iterator_traits<Iter>::value_type>
{
  ...
  template <typename U>
  bool operator!=(sequence_adapter_iterator<U> const & that) const
  {
    return ! (*this == that);
  }
...
template <typename C, typename F>
filter_iterator<typename C::iterator, F> 
filter(C c, F f)
{
  typedef filter_iterator<typename C::iterator, F> filter_t;
