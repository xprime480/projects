...
print(list(zip(count(1), Bango(1, 100, func1))))
print(list(gen_f(1, 100, func1)))

print(list(map(operator.pow, range(1,5), repeat(3))))
...
print([list(g)                          # show segments of large/small
       for k,g in groupby(chain(s1,s2), tiny)])
print(list(filter(tiny, chain(s1,s2)))) # show only small
print(list(filterfalse(tiny,           # show only large
                        chain(s1,s2))))
print(list(map(operator.mul, s1, s2)))  # multiply corresponding parts
print(list(islice(i2, 3)))               # first 3
print(list(islice(i2, 2, 5)))            # items 2..5
print(list(islice(i3, 18, 27, 2)))       # every other item from 18..27
print(list(zip(i2, count(1),           # combine 3 ranges
                range(10))))
...
#
print(list([x*x + 3*x + 4 for x in islice(count(1), 5)]))
